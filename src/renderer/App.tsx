...
function App() {
  ...
  useEffect(() => {
    ...
    const loadPluginsInit = async () => {
      ...
      if (window.electron?.ipcRenderer) {
        ...
        try {
          ...
          logger.info("[App] Found installed npm plugins", {
            count: installedPlugins.length,
          });

          for (const plugin of installedPlugins) {
            try {
              await remotePluginLoader.installPlugin(
                plugin.packageName,
                plugin.version,
              );
              logger.info(`[App] npm plugin loaded: ${plugin.id}`);
            } catch (error) {
              logger.error(`[App] Failed to load npm plugin: ${plugin.id}`, {
                error,
              });
            }
          }
        ...
        window.electron.ipcRenderer
          .invoke(window.electron.channels?.PLUGIN_LIST || "plugin:list")
          .then((mainProcessPlugins: Plugin[]) => {
            logger.info(
              `[App] Loaded ${mainProcessPlugins.length} plugins from main process`,
            );
            ...
            logger.info(
              `[App] Loaded ${registryPlugins.length} plugins from registry`,
            );
            ...
            logger.info(
              `[App] Total plugins after merge: ${allPlugins.length}`,
            );
            setPlugins(allPlugins);
          })
          .catch((error) => {
            logger.error("[App] Failed to load plugins from main process", {
              error,
            });
            ...
            logger.info(
              `[App] Loaded ${registeredPlugins.length} plugins from registry (fallback)`,
            );
        ...
        try {
          const installedPlugins = storageService.getInstalledRemotePlugins();
          logger.info("[App] Found installed remote plugins", {
            count: installedPlugins.length,
          });
          ...
          for (const plugin of installedPlugins) {
            try {
              ...
              logger.error(`[App] Failed to load remote plugin: ${plugin.id}`, {
                error,
              });
            }
          }
        ...
        logger.info(
          `[App] Web mode: Loaded ${registeredPlugins.length} plugins from registry`,
        );
        setPlugins(registeredPlugins);
      }
    };
    ...
    const handlePluginLoaded = (event: any, ...args: any[]) => {
      logger.info(
        `[App] Received plugin event: ${event}, args: ${JSON.stringify(args)}`,
      );
      loadPluginsInit();
    };
    ...
    const cleanupEventBus = eventBus.on(
      AppEvents.PLUGINS_CHANGED,
  ...
  const handleSearchEnter = useCallback(() => {
    if (filteredPlugins.length > 0) {
      ...
      const firstPlugin = filteredPlugins[0];
      if (firstPlugin) {
        handlePluginClick(firstPlugin.id);
      }
    }
  }, [filteredPlugins]);
        ...
        try {
          ...
          for (const plugin of installedPlugins) {
            try {
              ...
              logger.error(`Failed to reload npm plugin: ${plugin.id}`, {
                error,
              });
            }
          }
