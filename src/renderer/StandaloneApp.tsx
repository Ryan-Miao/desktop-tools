...
function StandaloneApp() {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      try {
        ...
        logger.info("Loading npm plugins in standalone mode", {
        ...
        for (const plugin of installedPlugins) {
          try {
            ...
            logger.error(
              `Failed to load npm plugin in standalone: ${plugin.id}`,
              { error },
            );
          }
        }
      } catch (error) {
        logger.error("Failed to load npm plugins in standalone mode", {
          error,
        });
      }
  ...
  const loadPluginComponent = useCallback(async () => {
    ...
    try {
      ...
      logger.info(
        `[StandaloneApp] Plugin component loaded successfully: ${pluginId}`,
        {
  ...
  if (isLoading) {
    return (
      ...
      >
        <Loading />
        <p style={{ fontSize: "14px", opacity: 0.7 }}>正在加载插件组件...</p>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      ...
      >
        <h2>❌ 插件加载失败</h2>
        <p style={{ marginTop: "16px", color: "var(--error-color, #dc3545)" }}>
          {loadError.message}
        </p>
