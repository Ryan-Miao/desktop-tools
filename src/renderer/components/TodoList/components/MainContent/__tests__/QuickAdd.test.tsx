...
vi.mock("@renderer/components/TodoList/utils/naturalLanguageParser", () => ({
  parseNaturalLanguageInput: (input: string) => {
    if (input.includes("明天")) {
      ...
      return {
        text: text || input,
        priority: undefined, // Don't override manual selection
...
describe("QuickAdd Component", () => {
  ...
  it("should respect advanced options", async () => {
    ...
    // Find priority select using role and label
    ...
    // Select 'high' priority using selectOptions
    ...
    // Verify the todo was created with high priority
    await waitFor(
      () => {
        const store = useTodoStore.getState();
