...
type ActivityEventType = 'CREATED' | 'UPDATED_TITLE' | 'UPDATED_CONTENT' | 'DELETED';
...
function Notepad({ onClose, onMinimize, onMaximize }: NotepadProps) {
  ...
  const createNote = useCallback(() => {
    ...
    const newNote = addActivityToNote(
      baseNote,
      'CREATED',
      '创建了笔记',
      {
    ...
    setNotes(prev =>
      prev.map(note => {
        if (note.id === selectedId) {
          let updatedNote = { ...note, title, content, updatedAt: new Date().toISOString() };
          ...
          if (note.content !== content && content.length > 0) {
            ...
            if (oldContentTrimmed !== newContentTrimmed && newContentTrimmed.length > 0) {
              ...
              updatedNote = addActivityToNote(
                ...
                {
                  field: 'content',
                  oldValue: oldContentTrimmed.slice(0, 50) + (oldContentTrimmed.length > 50 ? '...' : ''),
                  newValue: newContentTrimmed.slice(0, 50) + (newContentTrimmed.length > 50 ? '...' : ''),
                }
              );
            }
          }
          ...
          <div className="notepad-list">
            {notes.length === 0 ? (
              ...
              notes.map(note => (
                ...
                >
                  <div className="notepad-item-title">{note.title || '无标题'}</div>
          ...
          {selectedNote ? (
            <>
                  ...
                  <div className="notepad-content-preview">
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        h1: ({ children }) => <h1 style={{ fontSize: '1.8em', fontWeight: 'bold', marginBottom: '0.5em', borderBottom: '2px solid var(--border)', paddingBottom: '0.3em' }}>{children}</h1>,
                        h2: ({ children }) => <h2 style={{ fontSize: '1.5em', fontWeight: 'bold', marginBottom: '0.5em', borderBottom: '1px solid var(--border)', paddingBottom: '0.2em' }}>{children}</h2>,
                        h3: ({ children }) => <h3 style={{ fontSize: '1.3em', fontWeight: 'bold', marginBottom: '0.5em' }}>{children}</h3>,
                        h4: ({ children }) => <h4 style={{ fontSize: '1.1em', fontWeight: 'bold', marginBottom: '0.5em' }}>{children}</h4>,
                        ...
                        pre: ({ children }) => <pre style={{ background: '#f6f8fa', padding: '1em', borderRadius: '6px', overflowX: 'auto', marginBottom: '1em' }}>{children}</pre>,
                        a: ({ href, children }) => <a href={href} style={{ color: '#0366d6', textDecoration: 'underline' }} target="_blank" rel="noopener noreferrer">{children}</a>,
                        ...
                        hr: () => <hr style={{ border: 'none', borderTop: '2px solid #e1e4e8', margin: '1.5em 0' }} />,
                        table: ({ children }) => <div style={{ overflowX: 'auto', marginBottom: '1em' }}><table style={{
                        ...
                        strong: ({ children }) => <strong style={{ fontWeight: '600', color: '#24292e' }}>{children}</strong>,
                ...
                {showHistory && (
                  <div className="notepad-history-panel">
                    ...
                    <div className="notepad-history-list">
                      {selectedNote.activityHistory && selectedNote.activityHistory.length > 0 ? (
                        selectedNote.activityHistory.map((activity) => (
                          <div key={activity.id} className="notepad-history-item">
                              ...
                              <div className="notepad-history-changes">
                                {activity.changes.oldValue && (
                                  <div className="notepad-history-change">
                                    <span className="notepad-history-change-label">旧值:</span>
                                    <span className="notepad-history-change-old">{activity.changes.oldValue}</span>
                                  </div>
                                )}
                                {activity.changes.newValue && (
                                  <div className="notepad-history-change">
                                    <span className="notepad-history-change-label">新值:</span>
                                    <span className="notepad-history-change-new">{activity.changes.newValue}</span>
                                  </div>
                                )}
                              </div>
                            )}
              ...
              <div className="notepad-status">
                自动保存于 {new Date(selectedNote.updatedAt).toLocaleTimeString()}
              </div>
            </>
          ) : (
