...
const PluginList: React.FC<PluginListProps> = ({
  ...
  }, [layoutMode, gridColumns, hideTimer]);
  ...
  // 延迟隐藏 tooltip（鼠标离开卡片时）
  const handleMouseLeaveCard = useCallback((pluginId: string) => {
    const timer = setTimeout(() => {
      setHoveredPluginId(null);
    }, 150); // 150ms 延迟，允许鼠标移动到 tooltip
    setHideTimer(timer);
  }, []);

  // 立即显示 tooltip（鼠标悬停卡片时）
  ...
  const renderPluginCard = (plugin: Plugin, isFavorite: boolean = false) => {
    ...
    return (
      <div
        ...
        onMouseEnter={() => handleMouseEnterCard(plugin.id)}
        onMouseLeave={() => handleMouseLeaveCard(plugin.id)}
      ...
      >
        ...
        <div className="search-result-content">
          <div className="search-result-header">
            ...
            <span
              className={`search-result-favorite ${isFav ? "is-favorite" : ""}`}
            >
              {isFav ? "⭐" : "☆"}
            </span>
          </div>
