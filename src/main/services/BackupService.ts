...
export class BackupService {
   ...
   * 创建选择性备份
   */
  async createBackup(options: BackupOptions = {}): Promise<{ success: boolean; filePath?: string; error?: string }> {
    try {
      // 显示保存对话框
      ...
      try {
        ...
        if (options.includePlugins !== false) {
          ...
          if (options.pluginIds && options.pluginIds.length > 0) {
            pluginList = pluginList.filter(p => options.pluginIds!.includes(p.plugin_id));
          }

          // 导出每个插件的数据
          for (const plugin of pluginList) {
            const pluginData = this.db.getPluginData(plugin.plugin_id) as { plugin_id: string; plugin_name: string; plugin_version: string; data_json: string } | null;
            if (pluginData) {
              const pluginFile = path.join(pluginsDir, `${plugin.plugin_id}.json`);
  ...
  private async getAppSettingsFromRenderer(): Promise<any> {
    return new Promise((resolve) => {
      ...
      if (windows.length > 0 && !windows[0].isDestroyed()) {
        ...
        windows[0].webContents.send('backup:request-app-settings');
   ...
   * 预览备份文件内容
   */
  async previewBackup(backupPath: string): Promise<{ success: boolean; manifest?: BackupManifest; pluginData?: any[]; error?: string }> {
    ...
    try {
      ...
      if (fs.existsSync(manifestPath)) {
        ...
        if (fs.existsSync(pluginsDir)) {
          ...
          for (const file of pluginFiles) {
            if (file.endsWith('.json')) {
              ...
              const pluginContent = JSON.parse(fs.readFileSync(pluginFilePath, 'utf-8'));
              pluginData.push({
        ...
        // 旧版本格式，转换
        const oldInfo = JSON.parse(fs.readFileSync(path.join(tempDir, 'backup-info.json'), 'utf-8'));
        manifest = {
  ...
  async restoreBackup(options?: {
    ...
    try {
      ...
      try {
          ...
          if (fs.existsSync(pluginsDir)) {
            for (const pluginId of options.pluginIds) {
              ...
              if (fs.existsSync(pluginFile)) {
                const pluginContent = JSON.parse(fs.readFileSync(pluginFile, 'utf-8'));
                // 保存到数据库
        ...
        // 恢复应用设置
        if (isNewFormat && manifest.contents.appSettings && options?.includeAppSettings !== false) {
          ...
          if (fs.existsSync(appSettingsPath)) {
            ...
            const windows = BrowserWindow.getAllWindows();
            if (windows.length > 0 && !windows[0].isDestroyed()) {
              windows[0].webContents.send('backup:restore-app-settings', appSettings);
            }
          }
        }
