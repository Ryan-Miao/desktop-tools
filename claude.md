# Desktop Tool - AI 开发入口

> ⚠️ **强制执行规则**
>
> 以下规则是**强制性**的，不是"建议"或"最佳实践"：
>
> 1. **修改任何代码前**，必须先进行变更分类
> 2. **单次修改**不得超过 3 个文件
> 3. **核心文件**修改前必须通知用户
> 4. **开发流程**必须完成三个检查点：设计方案 → 测试验证 → 复盘汇报
> 5. **提交前**必须验证修复有效
>
> 违反以上规则将导致代码被回滚。

---

## ⚠️ 修改前强制检查

**在修改任何代码之前，必须按顺序完成以下检查**：

### 检查 1：变更分类（必须执行）

```
修改 src/main/ 或 src/shared/？
├─ 是 → Critical 变更
│   ├─ 必须通知用户
│   ├─ 必须获得用户批准
│   └─ 需要 80% 测试 + E2E + 性能
└─ 否
    ├─ 破坏性变更？ → Critical
    ├─ 新功能？ → Major
    ├─ Bug修复？ → Minor
    └─ 其他 → Trivial
```

**如何判断**：

- 检查要修改的文件路径
- 检查是否修改 API 接口
- 检查是否影响现有功能

**示例**：

- 修改 `tsconfig.json` → Critical
- 修改 `src/main/plugins/manager.ts` → Critical
- 修改组件样式 → Trivial

### 检查 2：功能开发范围（必须执行）

**禁止**：

- ❌ 一次修改超过 3 个文件
- ❌ 将多个独立功能合并为一次提交
- ❌ 使用 agents 并行修改多个文件

**功能开发定义**：

一次功能开发 = 完成一个**独立的、可测试的**功能单元

**独立功能的特征**：

- ✅ 单一职责：只做一件事
- ✅ 可独立验证：能单独运行测试
- ✅ 边界清晰：有明确的输入输出
- ✅ 价值完整：对用户有意义的价值增量

**示例**：

- ✅ "添加用户登录功能"（一次功能开发）
- ✅ "添加 FileStorageService 测试"（一次功能开发）
- ❌ "添加登录+注册+密码找回"（多个功能，应拆分）

**如果功能涉及多个文件**：

- 确保这些文件都属于同一个功能单元
- 每个文件的修改都服务于同一目标
- 一次提交不超过 3 个文件
- 如果超过 3 个文件，需要拆分为多个功能单元

### 检查 3：核心文件保护（必须执行）

**受保护的文件**：

- `tsconfig.json`
- `vite.config.ts`
- `package.json`
- `src/main/**/*.ts`
- `src/shared/**/*.ts`

**修改规则**：

1. 必须先进行变更分类
2. 必须通知用户变更级别
3. 必须获得用户明确同意
4. 必须先写测试（TDD）

**示例通知**：

```
变更级别：Critical

修改内容：
- tsconfig.json（关闭严格模式）
- 34 个组件文件

风险：
- 降低类型检查标准
- 可能引入运行时错误

建议：
- 选项 A：遵循 Critical 变更流程（设计审批 + 80% 测试）
- 选项 B：分批次渐进式修复（每次 2-3 个文件）
- 选项 C：降低为 Minor 变更（只修复阻塞 CI 的错误）

请选择方案：A / B / C
```

**只有获得用户明确同意后才能开始修改。**

---

## 📋 开发流程检查点（强制执行）

**每个功能开发必须完成以下三个检查点**：

### 检查点 1：开发前 - 设计方案评估（必须执行）

**时机**：在开始编写代码之前

**输出**：设计方案文档

**内容包括**：

```markdown
## 功能设计方案

### 1. 功能概述

- 功能名称：
- 功能目标：
- 用户价值：

### 2. 变更分类

- 变更级别：Critical / Major / Minor / Trivial
- 理由：

### 3. 技术方案

- 实现方案：
- 涉及文件：
- 依赖关系：

### 4. 测试策略

- 测试类型：单元测试 / 集成测试 / E2E
- 覆盖率目标：
- 测试重点：

### 5. 风险评估

- 潜在风险：
- 缓解措施：
- 回滚方案：

### 6. 验收标准

- 功能验收：
- 性能要求：
- 覆盖率要求：
```

**审批要求**：

- Minor/Trivial：自动审批
- Major/Critical：需要用户审批

---

### 检查点 2：开发后 - 测试覆盖率验证（必须执行）

**时机**：在代码实现完成后，提交之前

**验证项目**：

```bash
# 1. 类型检查
npm run type-check
# 预期：0 错误

# 2. 代码检查
npm run lint
# 预期：0 错误（警告可接受）

# 3. 单元测试
npm test
# 预期：所有测试通过

# 4. 覆盖率检查
npm run test:coverage
# 预期：
# - 核心代码：>= 85%
# - UI 组件：>= 60%
# - 其他：>= 80%
```

**输出**：测试覆盖率报告

```markdown
## 测试覆盖率报告

### 代码覆盖率

- 行覆盖率：XX%
- 分支覆盖率：XX%
- 函数覆盖率：XX%
- 语句覆盖率：XX%

### 核心代码覆盖率

- renderer/services：XX%
- shared/logger：XX%
- shared/types：XX%

### 测试结果

- 测试文件数：XX
- 测试用例数：XX
- 通过率：100%

### 未覆盖代码分析

- 未覆盖文件：
- 未覆盖原因：
- 改进计划：
```

**通过标准**：

- ✅ 所有测试通过
- ✅ 核心代码覆盖率 >= 85%
- ✅ 类型检查通过
- ✅ Lint 检查通过

---

### 检查点 3：完成后 - 复盘汇报（必须执行）

**时机**：在功能正常运行，没有异常之后

**触发条件**：

- 所有测试通过
- 代码已提交
- 应用运行无异常
- 功能已验证

**输出**：复盘汇报文档

```markdown
## 功能开发复盘汇报

### 一、功能总结

- **功能名称**：
- **开发时间**：
- **变更级别**：
- **涉及文件**：

### 二、规范遵循情况

#### 2.1 遵循的规范

- ✅ 变更分类：[级别]
- ✅ 功能开发单元：单一职责 / 独立功能 / 可测试
- ✅ 文件数量控制：X 个文件（符合 ≤ 3 要求）
- ✅ 提交前验证：type-check / lint / test / 覆盖率

#### 2.2 检查点完成情况

| 检查点                  | 状态         | 说明 |
| ----------------------- | ------------ | ---- |
| 检查点1：设计方案评估   | ✅ / ⚠️ / ❌ |      |
| 检查点2：测试覆盖率验证 | ✅ / ⚠️ / ❌ |      |
| 检查点3：复盘汇报       | ✅           |      |

### 三、测试覆盖率

#### 3.1 覆盖率数据

- **全局覆盖率**：XX%
- **核心代码覆盖率**：XX%
- **新增测试文件**：X 个
- **新增测试用例**：X 个

#### 3.2 覆盖率目标达成情况

| 代码类型          | 目标   | 实际 | 状态    |
| ----------------- | ------ | ---- | ------- |
| renderer/services | >= 85% | XX%  | ✅ / ❌ |
| shared/logger     | >= 85% | XX%  | ✅ / ❌ |
| shared/types      | >= 85% | XX%  | ✅ / ❌ |

### 四、问题与改进

#### 4.1 未做到的规范项

如果有规范项未完全遵循，说明：

**问题描述**：

- 具体问题：
- 影响范围：
- 严重程度：

**原因分析**：

- 根本原因：
- 客观限制：
- 主观因素：

**改进措施**：

- 即时措施：
- 长期方案：
- 预防措施：

#### 4.2 开发过程中的问题

**问题1**：

- 问题描述：
- 解决方案：
- 经验教训：

**问题2**：
...

### 五、经验总结

#### 5.1 做得好的地方

1. ...
2. ...

#### 5.2 可以改进的地方

1. ...
2. ...

#### 5.3 技术债务记录

- 新增技术债务：
- 计划解决时间：

### 六、下一步计划

- [ ] 待完成功能
- [ ] 需要优化项
- [ ] 后续跟进事项

---

**汇报人**：Claude AI
**汇报时间**：
**审核状态**：待审核 / 已通过
```

**汇报提交**：

- 保存到：`devdoc/summaries/[功能名称]-[日期].md`
- 提交到 git（可选）

---

## ⚠️ 提交前强制验证

**在执行 `git commit` 之前，必须完成以下验证**：

### 验证 1：编译通过

```bash
npm run type-check  # 必须通过，0 错误
```

### 验证 2：Lint 通过

```bash
npm run lint  # 必须通过，0 错误（警告可接受）
```

### 验证 3：测试通过

```bash
npm test  # 必须通过，所有测试用例
```

### 验证 4：应用能启动（关键）

```bash
# 如果开发服务器未运行，启动它
npm run dev

# 等待启动完成
# 在浏览器中访问应用
# 检查是否有运行时错误
```

**如果修复了 bug，必须验证**：

1. 应用能正常启动
2. bug 已解决（在浏览器中验证）
3. 没有引入新的 bug
4. 控制台没有错误

### 验证 5：检查修改范围

```bash
git diff --stat  # 查看修改统计
```

**检查**：

- ✅ 文件数 ≤ 3（或之前获得批准）
- ✅ 属于同一个功能单元
- ✅ 不包含意外修改的文件

**功能单元自检**：

1. 这次提交是否只完成一件事？
2. 这些文件的修改是否服务于同一目标？
3. 能否用一个简洁的提交信息描述？

**如果超出范围**：

- 取消提交
- 重新规划功能单元边界
- 确保每个提交都是独立完整的功能

**只有以上所有验证都通过后，才能执行 git commit。**

---

## 🚫 明确禁止的操作

以下操作被**严格禁止**，任何情况都不得执行：

### 禁止 1：批量修改

**禁止**：

- ❌ 一次修改超过 3 个文件（除非是同一功能单元）
- ❌ 将多个独立功能合并为一次提交
- ❌ 使用 Task 工具并行修改多个文件
- ❌ 使用多个 agents 同时修改代码

**什么是批量修改（示例）**：

- ❌ 一次提交包含 3 个独立的 bug 修复
- ❌ 一次提交包含 2 个不相关的功能开发
- ❌ 一次提交修改了 5 个以上的文件（极少数情况除外）

**什么是可接受的多文件修改（示例）**：

- ✅ 添加一个功能，涉及接口 + 实现 + 测试（3 个文件）
- ✅ 修复一个 bug，涉及组件 + 样式 + 测试（3 个文件）
- ✅ 添加测试，涉及多个测试文件（如果是同一个测试任务）

**后果**：

- 立即回滚
- 需要重新制定计划
- 严重违反规范

### 禁止 2：跳过验证

**禁止**：

- ❌ 修改代码后不测试就提交
- ❌ 修复 bug 后不验证修复有效就提交
- ❌ 运行 `git commit` 前不运行应用
- ❌ 运行 `git push` 前不在本地测试

**后果**：

- 立即回滚
- 可能破坏主分支
- 影响所有开发者

### 禁止 3：未经批准修改核心文件

**禁止**：

- ❌ 修改 `tsconfig.json` 前不通知用户
- ❌ 修改 `vite.config.ts` 前不通知用户
- ❌ 修改 `src/main/**/*.ts` 前不通知用户
- ❌ 修改 `src/shared/**/*.ts` 前不通知用户

**正确流程**：

1. 进行变更分类
2. 通知用户变更级别
3. 说明风险和影响
4. 获得用户明确同意
5. 然后才能开始修改

### 禁止 4：降低质量标准

**禁止**：

- ❌ 为了"快速修复"而关闭 TypeScript 严格选项
- ❌ 为了"通过 CI"而降低测试覆盖率要求
- ❌ 为了"节省时间"而跳过代码审查

**后果**：

- 技术债累积
- 长期维护成本增加
- 违反项目价值观

---

## 快速导航

### 开发规范 (必须阅读)

- **核心规范**: `devdoc/ai/claude.md` - TDD、代码规范、提交规范
- **测试指南**: `devdoc/standards/testing.md`
- **变更分类**: `devdoc/standards/change_classification.md`
- **快速参考**: `devdoc/standards/quick_reference.md`

### 项目文档

- **项目说明**: `README.md`
- **插件开发**: `doc/PLUGIN_DEVELOPMENT.md`
- **构建指南**: `doc/BUILD.md`
- **贡献指南**: `doc/CONTRIBUTING.md`

### 开发进度

- **当前状态**: `devdoc/progress/PROJECT_STATUS.md`
- **开发计划**: `devdoc/plans/task_plan.md`

---

## 核心开发规范

### 1. 变更分类决策树

```
修改 src/main/ 或 src/shared/？
├─ 是 → Critical (设计审批 + 80%测试)
└─ 否
    ├─ 破坏性变更？ → Critical
    ├─ 新功能？ → Major (设计审批 + 70%测试)
    ├─ Bug修复？ → Minor (复现测试)
    └─ 其他 → Trivial (现有测试)
```

### 2. TDD 测试规范

- **测试优先开发**（核心代码必须）
- **日志规范**: 使用 `createLogger`，禁用 `console.log`
- **CSS变量**: 所有样式必须使用 CSS 变量支持主题
- **内联确认**: 禁用 confirm/alert，使用内联确认 + Toast

### 3. 提交规范

```
<type>(<scope>): <subject>

类型: feat, fix, docs, style, refactor, perf, test, chore
示例: feat(plugin): add hot reload feature
```

### 4. 常用命令

```bash
# 开发
npm run dev              # 启动开发服务器
npm start                # 启动 Electron

# 测试
npm test                 # 运行所有测试
npm run test:coverage    # 生成覆盖率报告

# 代码检查
npm run type-check       # TypeScript 类型检查
npm run lint             # ESLint 检查
```

---

## 文档组织规范

| 目录      | 用途       | 示例                              |
| --------- | ---------- | --------------------------------- |
| `doc/`    | 用户文档   | README, PLUGIN_DEVELOPMENT, BUILD |
| `devdoc/` | 开发者文档 | 规范、计划、进度、总结            |

### 用户文档 (`doc/`)

- README.md - 项目介绍和使用指南
- PLUGIN_DEVELOPMENT.md - 插件开发指南（面向插件作者）
- BUILD.md - 构建和打包说明
- CONTRIBUTING.md - 贡献指南

### 开发者文档 (`devdoc/`)

- `ai/` - AI 开发规范
- `standards/` - 开发标准（测试、代码审查等）
- `plans/` - 开发计划
- `progress/` - 项目进度
- `summaries/` - 开发总结和报告

---

## 核心代码路径

**必须满足更高测试覆盖率的核心代码**：

```
src/main/              # 主进程
  ├── index.ts
  ├── database/
  ├── plugins/
  ├── windows/
  ├── ipc/
  └── services/

src/shared/
  ├── types/          # 类型定义
  └── logger/         # 日志框架

src/renderer/services/
  └── StorageService.ts
```

---

## 快速参考

### 变更类型快速判断

| 类型         | 标准             | 测试要求 | 设计审批  |
| ------------ | ---------------- | -------- | --------- |
| **Critical** | 核心、破坏、安全 | 80%+E2E  | ✅ 必需   |
| **Major**    | 新功能、架构     | 70%+集成 | ✅ 必需   |
| **Minor**    | Bug修复、小增强  | 复现测试 | ❌ 不需要 |
| **Trivial**  | UI、配置         | 现有测试 | ❌ 不需要 |

---

## 下一步

1. **阅读完整规范**: `devdoc/ai/claude.md` - 了解 TDD、日志、代码规范等详细内容
2. **查看当前状态**: `devdoc/progress/PROJECT_STATUS.md` - 了解项目当前进度
3. **查找具体标准**: 根据任务类型查找 `devdoc/standards/` 中的对应文档

---

**版本**: 3.0 - 检查点与复盘版本
**最后更新**: 2026-01-23
**重大变更**：

- v2.0: 从"建议"改为"强制"，添加明确的禁止规则
- v2.1: 取消行数限制，改为"一次功能开发"概念，强调功能单元边界
- v3.0: **新增开发流程检查点**（设计方案评估 → 测试覆盖率验证 → 复盘汇报）
